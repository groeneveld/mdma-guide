...
\newglossaryentry{reconsolidation}{
  ...
  description={When a schema/implicit-memory first forms, it is \textit{consolidated}~\cite{eckerUnlocking}. When contradictory evidence causes sufficient mismatch or prediction error, the schema enters a state where it can be changed (it becomes \textit{labile}). Maintaining that mismatch or prediction error gradually updates the schema to reflect the new information. At the end of this process the schema \textit{re-consolidates,} and becomes unchangeable again. I use the term \textit{reconsolidate} to refer to this whole process of activation, updating, and reconsolidation.}
