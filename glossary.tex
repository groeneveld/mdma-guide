...
\newglossaryentry{attractor-state}{
  name={Attractor},
  description={A state in a complex system that the system is drawn to and which it is difficult to get out of~\cite{hayes2020complex}. Many mental illnesses are attractor states. Stuck schemas seem to stick around when they're paired with some reconsolidation-inhibiting process (like avoidance), which together form an attractor state~\cite{hayes2020complex,berghBSTS}. The durability of the state requires multiple reinforcing elements; removing those elements destabilizes it.}
  %defined_in={sec:complex},
  %stem={attractor}
}
